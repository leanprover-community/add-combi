...
This file proves a version the Marcinkiewicz-Zygmund inequality, which bounds the L^p norm of a sum
of independent random variables in terms of the expectation of their pointwise L^2 norm.

There are a few caveats to our current statement:
* We assume the domain of our random variables is finite and represent their expectation as a
  literal sum
* We assume that `p` is an even integer. This can be lifted after the fact using nesting of
  L^p norms.

## TODO

Remove the useless annotations like `(2 : ℕ)` once
https://leanprover.zulipchat.com/#narrow/channel/270676-lean4/topic/Elaboration.20of.20heterogeneous.20mul.20with.20the.20module.20system/with/592236603
is resolved.
-/
...
/-- A version of the **Marcinkiewicz-Zygmund inequality** for real-valued functions with
finite domain and for `p := 2 * m` an even integer, with a slightly better constant than
`Real.marcinkiewicz_zygmund`. -/
theorem marcinkiewicz_zygmund' (m : ℕ) (f : ι → ℝ) (hf : ∀ i, ∑ a ∈ A ^^ n, f (a i) = 0) :
    ∑ a ∈ A ^^ n, (∑ i, f (a i)) ^ ((2 : ℕ) * m) ≤
      ((4 : ℝ) * m) ^ m * ∑ a ∈ A ^^ n, (∑ i, f (a i) ^ 2) ^ m := by
...
/-- A version of the **Marcinkiewicz-Zygmund inequality** for real-valued functions with
finite domain and for `p := 2 * m` an even integer, with a slightly easier to bound constant than
`Real.marcinkiewicz_zygmund'`.
...
theorem marcinkiewicz_zygmund (hm : m ≠ 0) (f : ι → ℝ) (hf : ∀ i, ∑ a ∈ A ^^ n, f (a i) = 0) :
    ∑ a ∈ A ^^ n, (∑ i, f (a i)) ^ ((2 : ℕ) * m) ≤
      ((4 : ℝ) * m) ^ m * n ^ (m - 1) * ∑ a ∈ A ^^ n, ∑ i, f (a i) ^ ((2 : ℕ) * m) := by
  obtain _ | m := m
  · simp at hm
...
/-- A version of the **Marcinkiewicz-Zygmund inequality** for real- or complex-valued functions with
finite domain and for `p := 2 * m` an even integer. -/
lemma marcinkiewicz_zygmund (hm : m ≠ 0) (f : ι → 𝕜) (hf : ∀ i, ∑ a ∈ A ^^ n, f (a i) = 0) :
    ∑ a ∈ A ^^ n, ‖∑ i, f (a i)‖ ^ ((2 : ℕ) * m) ≤
      ((8 : ℝ) * m) ^ m * n ^ (m - 1) * ∑ a ∈ A ^^ n, ∑ i, ‖f (a i)‖ ^ ((2 : ℕ) * m) := by
  let f₁ x : ℝ := re (f x)
  let f₂ x : ℝ := im (f x)
  let B := A ^^ n
